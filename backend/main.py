...
# 이 프로젝트는 매일 쏟아지는 수백 건의 공공기관 제안요청서(RFP)를 인공지능이 대신 읽고 요약해 주는 아주 훌륭하고 실용적인 B2G 컨설팅 지원 도구네요! 이전 코드(chatbot_original.py)가 사용자와 직접 만나는 '웹사이트 화면(프론트엔드)'이었다면, 이번에 보여주신 main.py 코드는 그 화면 뒤에서 실제로 질문을 받고 묵묵히 대답을 만들어내는 '서버(백엔드)'를 만드는 코드입니다.
